...
## 更新摘要
**所做更改**
- 简化了模块结构，移除了冗余的文档文件（_module.yaml、架构设计.md、概述.md）
- 更新了文档以反映当前的扁平化结构
- 集成了新的 PointInfoView 控件用于点位信息显示
- 保持了四轴系统支持的完整功能
...
- 完整的工具类库（MathHelper、PaintHelper）的使用与扩展
- U轴旋转控制和URotationView可视化控件
- **新增**：PointInfoView控件用于点位信息的显示和管理
- 生命周期管理与事件处理机制
- 错误处理策略与异常恢复方法
- 最佳实践与性能优化建议
...
## 项目结构
MainControl 子项目已重构为完全独立的DLL项目，采用扁平化的文件组织结构，包含UI表单、设置类、进程模块入口以及完整的工具类库。关键目录与文件如下：
...
- Controls 目录：共享控件（URotationView、PointInfoView等）
- 资源与构建：MainControl.csproj
...
P["PlatformMotionService.cs"]
Q["PlatformMotionAdapter.cs"]
R["IMotionService.cs"]
S["AxisController.cs"]
T["XyzControllerHub.cs"]
end
A --> D
A --> E
A --> F
...
- 设置类：
  ...
  - MainControlProjectSetting：项目级参数（如轴映射、点位表、工艺参数等），支持四轴配置
- 完整工具类库：
  ...
  - ZBarView：Z轴条形视图控件
  - URotationView：U轴旋转角度可视化控件
  - **新增**：PointInfoView：点位信息展示控件，用于显示和管理点位数据
- 运动服务：
  - PlatformMotionService：对外暴露的运动控制服务，封装命令编排与状态同步，支持四轴协调
  - PlatformMotionAdapter：平台适配层，屏蔽不同硬件差异
  - IMotionService：运动服务接口定义
  - AxisController / XyzControllerHub：轴控制器与XYZ三轴协调器，支持四轴
...
## 四轴系统支持
MainControl模块完全支持四轴系统（X、Y、Z、U轴），包括以下功能：
...
## 共享控件库
MainControl模块包含专门的Controls目录，包含可重用的UI控件和工具类，供各个界面组件共享使用。
...
## 架构总览
MainControl 采用"界面-服务-适配器"的分层架构，并通过完整的工具类库实现功能的复用。支持四轴系统和PointInfoView控件的集成：
- 界面层：UnifiedRunForm、RunForm、AxisLimitForm、URotationForm 负责用户交互与参数输入
- 工具层：MathHelper、PaintHelper等工具类提供基础计算和绘图功能
- 控件层：DroLabel、JogButton、XYView、URotationView、PointInfoView等共享控件提供基础UI功能
- 服务层：PlatformMotionService 提供统一的运动控制 API，处理并发、状态机与事件，支持四轴协调
- 适配层：PlatformMotionAdapter 对接具体设备驱动，屏蔽差异
- 控制器：AxisController、XxyzControllerHub 实现轴级与多轴协同控制，支持四轴
...
UnifiedRunForm --> PointInfoView : "使用"
RunForm --> DroLabel : "使用"
...
URotationForm --> PointInfoView : "使用"
...
- 使用完整的工具类库中的MathHelper、PaintHelper等组件
- 支持四轴协调运动和U轴控制
...
- 硬限位优先级高于软限位
- 修改后立即生效并写入持久化配置
- 通过DroLabel实时显示当前限位值，使用MathHelper进行数值验证，四轴限位协调
...
### **新增** PointInfoView 点位信息控件
职责：
- 提供点位的结构化显示和编辑功能
- 支持多种数据类型（坐标、角度、速度、加速度等）
- 与运动控制系统的集成接口
- 实时数据绑定和状态更新
...
## 依赖关系分析
MainControl 对 Logic 层存在明确依赖，UI 不直接访问硬件，所有运动指令均通过服务层下发。完整的工具类库被所有UI组件引用，实现了功能的模块化，支持四轴系统。
...
## 结论
MainControl 模块通过清晰的层次划分、完善的设置/限位机制以及完整的工具类库和四轴系统支持，提供了稳定可靠的运动控制能力。模块现已完全独立，包含完整的四轴运动控制逻辑、UI控件和工具类，通过独立的DLL架构具有更好的可重用性和可维护性。开发者应遵循服务层抽象、事件驱动与异步执行的原则，结合合理的错误处理与性能优化策略，快速集成并扩展功能。

四轴系统支持使得模块能够处理更复杂的运动控制场景，特别是需要U轴旋转的应用场合。URotationForm和URotationView的引入为用户提供了直观的四轴控制界面。**新增的PointInfoView控件**进一步增强了点位数据的管理和显示能力。
...
- **新增**：PointInfoView使用
  - PointInfoView使用：[PointInfoView.cs](file://MainControl/Controls/PointInfoView.cs)
- 执行点动与绝对定位
